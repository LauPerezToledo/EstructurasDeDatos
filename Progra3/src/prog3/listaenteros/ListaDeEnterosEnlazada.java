...
public class ListaDeEnterosEnlazada extends ListaDeEnteros {
	...
	public ListaDeEnterosEnlazada ordenar(){
		...
		for (i = 0; i < this.tamanio; i++) {
			...
			menor = 99999;
			while(!this.fin()){
				if ((min < this.actual.getDato())&&(this.actual.getDato() < menor)){
					menor = this.actual.getDato();
				}
				this.proximo();
			}
		aux.agregarFinal(menor);
		min = menor;
	...
	public ListaDeEnterosEnlazada combinarOrdenado (ListaDeEnterosEnlazada listaParam) {
		...
		while (!this.fin() && !listaParam.fin()) {
			if (this.actual.getDato() < listaParam.actual.getDato()) {
				nue.agregarFinal(this.actual.getDato());
				this.proximo();
				
			}
				else{
					nue.agregarFinal(listaParam.actual.getDato());
					listaParam.proximo();
				}
		}
		if (!this.fin())
