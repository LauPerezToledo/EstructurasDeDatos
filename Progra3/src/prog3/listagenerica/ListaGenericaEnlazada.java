...
public class ListaGenericaEnlazada<T> extends ListaGenerica<T> {
	...
	public ListaGenericaEnlazada<T> invertir (){
		...
		if (this.fin())
			return nue;
				...
				nue.agregarInicio(this.actual.getDato());	
				this.proximo();
				invertir();
			}
		return nue;		
	...
	public boolean esBalanceado (String cadena) {
		...
		int mid, j = 0;
		ListaGenericaEnlazada<Character> nue = new ListaGenericaEnlazada<Character>();
		nue.comenzar();
		if (!(cadena.length() % 2 == 0)) {
			
			for (int i = 0; i < cadena.length(); i++) {
				nue.agregarFinal(cadena.charAt(i));			
			}
			mid = nue.tamanio / 2;
			while ((j < mid) && (nue.elemento(j) == nue.elemento(nue.tamanio - j)))
				j++;
			}
		if (nue.elemento(j) == nue.elemento(nue.tamanio - j))
			balanceado = true;
	return balanceado;
	}
}
